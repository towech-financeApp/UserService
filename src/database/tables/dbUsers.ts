...
export default class DbUsers {
  ...
  static add = async (name: string, username: string, password: string, role: string = 'user'): Promise<User> => {
    const response = await Query(
      `INSERT INTO Users(name, username, password, role) VALUES('${name}', '${username}', '${password}', '${role}') RETURNING *`,
    );
  ...
  static login = async (user: User): Promise<User> => {
    const response = await Query(
      `UPDATE Users SET refreshTokens = '{${user.refreshtokens}}', singleSessionToken = '${user.singlesessiontoken}' WHERE userID = ${user.userid} RETURNING *`,
    );
